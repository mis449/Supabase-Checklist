  ...
  try {
    ...
    return data.map(doneItem => {
      ...
      return {
        ...detail,
        ...doneItem,
        id: doneItem.task_id, // Map task_id to id for consistency
        done_id: doneItem.id,
...
export const updateDelegationTask = createAsyncThunk(
  'delegation/updateTask',
  async ({ id, task_description }, { rejectWithValue }) => {
    try {
      console.log('Updating delegation task:', { id, task_description });
      const { data, error } = await supabase
        .from('delegation')
        .update({ task_description, updated_at: new Date().toISOString() })
        .eq('task_id', id)
        .select();
...
export const updateTaskDescriptionGeneric = createAsyncThunk(
  'tasks/updateDescription',
  async ({ tableName, id, idColumn, description, descriptionColumn }, { rejectWithValue }) => {
    try {
      console.log(`Updating ${tableName} task:`, { id, description });
      
      const updateData = { [descriptionColumn]: description };
      
      // Only add updated_at if the table supports it
      if (['delegation', 'ea_tasks'].includes(tableName)) {
      ...
      if (error) throw error;
      return { tableName, data: data[0], id };
    } catch (error) {
      console.error(`Error updating ${tableName} task:`, error);
      return rejectWithValue(error.message);
    }
  }
);
