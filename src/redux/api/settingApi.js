...
export const fetchUserDetailsApi = async (role, username) => {
  try {
    let query = supabase.from("users").select('*');

    if (role !== 'admin' && username) {
      console.log(`🔒 Non-admin user detected (${username}), filtering data...`);
    ...
    console.log(`✅ Fetched ${data?.length} users successfully for role: ${role}`);
    return data || [];
  } catch (error) {
    console.error("❌ Exception in fetchUserDetailsApi:", error);
    return [];
  }
};
