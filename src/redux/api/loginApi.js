...
export const LoginCredentialsApi = async (formData) => {
  try {
    console.log("🔍 Login Debug - Username:", formData.username);
    console.log("🔍 Login Debug - Password:", formData.password);
    console.log("🔍 Login Debug - Full Form Data:", formData);
    ...
    console.log("🔍 Debug - User exists in DB?:", userExists && userExists.length > 0 ? "YES" : "NO", userExists);

    const { data, error } = await supabase
      .from('users')
      .select('*')
      .ilike('user_name', formData.username)
      .eq('password', formData.password);

    console.log("🔍 Login Query Result Data:", data);
