...
export const fetchDashboardDataApi = async (
  ...
  try {
    ...
    const dateColumn = (dashboardType === 'checklist' || dashboardType === 'delegation' || dashboardType === 'maintenance' || dashboardType === 'ea' || dashboardType === 'repair') ? 'planned_date' : 'task_start_date';
    // Use ascending order for checklist/delegation/maintenance to show oldest/most overdue first
    const isAscending = (dashboardType === 'checklist' || dashboardType === 'delegation' || dashboardType === 'maintenance' || dashboardType === 'ea' || dashboardType === 'repair');
    ...
    switch (taskView) {
      case 'recent':
        ...
        if (dashboardType === 'checklist' || dashboardType === 'all_images' || dashboardType === 'delegation') {
...
export const getDashboardDataCount = async (dashboardType, staffFilter = null, taskView = 'recent', departmentFilter = null) => {
  try {
    ...
    const dateColumn = (dashboardType === 'checklist' || dashboardType === 'delegation' || dashboardType === 'all_images') ? 'planned_date' : 'task_start_date';
...
export const countPendingOrDelayTaskApi = async (dashboardType, staffFilter = null, departmentFilter = null) => {
  ...
  try {
    ...
    if (dashboardType === 'delegation') {
      query = supabase
        .from(dashboardType === 'delegation' ? 'delegation' : 'checklist')
        .select('*', { count: 'exact', head: true })
        .is('submission_date', null)
        .not('status', 'eq', 'done')
