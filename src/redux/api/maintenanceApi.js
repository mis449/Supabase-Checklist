        ...
        const updates = await Promise.all(submissionData.map(async (item) => {
            ...
            if (item.image && item.image.previewUrl) {
                try {
                    ...
                    const { error: uploadError } = await supabase.storage.from('maintenance_tasks').upload(fileName, file);

                    if (uploadError) throw uploadError;

                    const { data: { publicUrl } } = supabase.storage.from('maintenance_tasks').getPublicUrl(fileName);
                    imageUrl = publicUrl;
