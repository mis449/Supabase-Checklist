...
export const fetchUniqueDepartmentDataApi = async () => {
  try {
    ...
    const uniqueDepartments = data.map(item => item.name).filter(Boolean);
...
export const fetchUniqueDoerNameDataApi = async (department) => {
  try {
    ...
    // Fallback: If no users found for specific department, try fetching all users
    if (!error && (!data || data.length === 0) && department) {
      console.log("⚠️ No users found for department, fetching all users as fallback...");
