...
import { updateTaskDescriptionGeneric } from "../../redux/api/delegationApi";
...
const AllTasks = () => {
  ...
  const handleOpenEditModal = (task) => {
    setTaskToEdit(task);
    setEditedDescription(task.task_description || task.issue_description || "");
    setIsEditModalOpen(true);
  };

  const handleUpdateDescription = async () => {
    if (!taskToEdit || !editedDescription.trim()) return;
    
    setIsUpdating(true);
    try {
      let tableName = "";
      let idColumn = "task_id";
      let descriptionColumn = "task_description";
      ...
      switch(activeTab) {
        ...
        case "repair":
          ...
          idColumn = "id";
          descriptionColumn = "issue_description";
          break;
        case "ea":
          tableName = "ea_tasks";
          break;
      ...
      const resultAction = await dispatch(updateTaskDescriptionGeneric({
        tableName,
        id: taskToEdit.task_id || taskToEdit.id,
        idColumn,
        description: editedDescription,
        descriptionColumn
      }));

      if (updateTaskDescriptionGeneric.fulfilled.match(resultAction)) {
        showToast("Description updated successfully", "success");
        // Update local state
        setTasks(prev => prev.map(t => {
          const tId = t.task_id || t.id;
          const editId = taskToEdit.task_id || taskToEdit.id;
          if (tId === editId) {
            return { ...t, [descriptionColumn]: editedDescription };
          }
          return t;
        }));
        setIsEditModalOpen(false);
      } else {
        showToast(resultAction.payload || "Failed to update description", "error");
      }
    } catch (err) {
  ...
  const handleRepairUpdateSubmit = async (e) => {
    ...
    try {
      ...
      if (updateForm.workPhoto) {
        ...
        const { data, error } = await supabase.storage.from('all_images').upload(fileName, updateForm.workPhoto);
        if (error) throw error;
        const { data: { publicUrl } } = supabase.storage.from('all_images').getPublicUrl(fileName);
      ...
      if (updateForm.billCopy) {
        ...
        const { data, error } = await supabase.storage.from('all_images').upload(fileName, updateForm.billCopy);
        if (error) throw error;
        const { data: { publicUrl } } = supabase.storage.from('all_images').getPublicUrl(fileName);
                                                    ...
                                                    : (header.id === 'task_description' || header.id === 'issue_description' || header.id === 'remarks')
                                                      ? (
                                                        <div className="group relative">
                                                          ...
                                                          {userRole.toLowerCase() === 'admin' && (header.id === 'task_description' || header.id === 'issue_description') && (
                                                            <button
                                                              onClick={() => handleOpenEditModal(task)}
                                                              className="absolute -top-1 -right-1 p-1 bg-white border border-purple-200 rounded-full text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:bg-purple-50 z-10"
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">
                  Task ID: #{taskToEdit?.task_id || taskToEdit?.id}
                </label>
                <textarea
                  value={editedDescription}
                  onChange={(e) => setEditedDescription(e.target.value)}
                  className="w-full h-40 p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all resize-none text-gray-700 text-sm font-medium"
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => setIsEditModalOpen(false)}
                  className="flex-1 px-4 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-all active:scale-95"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-[2] px-4 py-3 bg-purple-600 text-white font-bold rounded-xl shadow-lg shadow-purple-200 hover:bg-purple-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isUpdating ? (
                    <>
