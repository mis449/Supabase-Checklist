...
export default function QuickTask() {
  ...
  useEffect(() => {
    ...
    if (role === "hod") {
      showToast("Access Denied: HODs cannot access Task Manager List management.", "error");
      navigate("/dashboard");
    }
  }, [navigate]);
  ...
  const handleScroll = useCallback(() => {
    if (!tableContainerRef.current || loading || (activeTab === 'all_images' && maintenanceLoading)) return;
    ...
    if (scrollHeight - scrollTop - clientHeight < 100) {
      ...
      } else if (activeTab === 'all_images' && maintenanceHasMore) {
  ...
  const handleEditClick = (task) => {
    ...
    if (activeTab === 'all_images') {
      ...
      setEditFormData({
        ...
        duration: task.duration || '',
        enable_reminder: task.enable_reminder ? 'yes' : 'no',
        require_attachment: task.require_attachment ? 'yes' : 'no',
        instruction_attachment_url: instructionUrls,
        instruction_attachment_type: instructionTypes,
        remark: task.remark || '',
        originalAudioUrl: task.audio_url || (isAudioUrl(task.task_description) ? task.task_description : null),
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Convert 'yes'/'no' strings back to booleans for the API
      if (finalEditData.enable_reminder !== undefined) {
        finalEditData.enable_reminder = finalEditData.enable_reminder === 'yes';
      }
      if (finalEditData.require_attachment !== undefined) {
        finalEditData.require_attachment = finalEditData.require_attachment === 'yes';
      }
  ...
  const handleSelectAll = () => {
    const currentTasks =
      activeTab === 'checklist' ? filteredChecklistTasks :
        activeTab === 'all_images' ? filteredMaintenance :
